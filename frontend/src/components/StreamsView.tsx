        ...
        <div className="animate-in duration-400 fade-in slide-in-from-right-4 space-y-6">
          <header className="sticky top-0 bg-white/95 backdrop-blur-md p-4 flex flex-col sm:flex-row sm:items-center justify-between border border-slate-200/80 rounded-2xl shadow-sm z-30 gap-4">
            ...
            <div className="flex items-center gap-2">
              ...
              <div className="flex bg-indigo-50 text-[#3525cd] px-3 py-1.5 rounded-xl items-center gap-1.5 text-xs font-black">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div className="bg-gradient-to-br from-[#4f46e5] to-[#3525cd] rounded-3xl p-6 text-white shadow-xl relative overflow-hidden">
              <div className="absolute -right-8 -top-8 w-32 h-32 bg-white/10 rounded-full blur-2xl" />
              <div className="relative z-10 space-y-4">
                <p className="text-[10px] font-bold tracking-widest uppercase text-indigo-100">Enrolment Summary</p>
                <div className="flex items-baseline gap-2">
                  <span className="text-4xl font-black tracking-tight">
                    {students.filter(s => s.streamId === activeStream.id).length}
                  </span>
                  <span className="text-xs text-indigo-100 font-semibold">Active Candidates</span>
                </div>
                <div className="pt-2 border-t border-white/20 flex justify-between text-[11px] text-indigo-100">
            ...
            <div className="bg-white p-5 rounded-3xl border border-slate-200/80 shadow-sm flex flex-col justify-between">
                ...
                <div className="flex items-center justify-between pt-1">
                  ...
                  {currentUser?.role === 'ADMIN' && (
                    <button
                      ...
                      className="p-1 hover:bg-slate-100 rounded-lg text-slate-500 hover:text-[#3525cd] cursor-pointer"
              ...
              <div className="text-[10px] text-[#3525cd] font-bold">Primary Guidance Counsellor</div>
            </div>
            ...
            <div className="flex flex-col sm:flex-row justify-between sm:items-center gap-4">
              ...
              <div className="relative w-full sm:w-64">
                ...
                <input
                  ...
                  className="w-full pl-9 pr-4 py-2 bg-slate-50 border border-slate-200 rounded-xl text-xs font-semibold text-slate-705 outline-none focus:border-[#3525cd] transition"
                />
              </div>
            </div>
                    ...
                    return (
                      <tr key={st.id} className="hover:bg-slate-50/50">
                        ...
                        <td className="p-3 text-center">
                          <span className="px-2.5 py-1 bg-indigo-50 text-[#3525cd] font-black rounded-lg text-[10px] font-mono border border-indigo-100">
                            {st.overallPosition ? `#${st.overallPosition}` : 'N/A'}
                          </span>
                        </td>
                        <td className="p-3">
                          <p className="text-slate-800 font-semibold">{st.parentName || 'N/A'}</p>
                          <p className="text-[9px] text-[#3525cd] font-bold font-mono">Guardian</p>
                        </td>
          ...
          {currentUser?.role === 'ADMIN' && (
            <div className="flex justify-between items-center bg-white p-4 border border-slate-200/80 rounded-2xl shadow-sm flex-col sm:flex-row gap-4">
              ...
              <button
                ...
                className="px-4 py-2 bg-[#3525cd] hover:bg-[#4f46e5] text-white rounded-xl text-xs font-bold transition-all cursor-pointer shadow-md shadow-indigo-600/10 active:scale-95"
              >
                Create Stream
              </button>
            </div>
            ...
            {streams.map(stream => {
              ...
              return (
                <div
                  ...
                  className="bg-white rounded-3xl border border-slate-200/80 p-6 shadow-sm hover:shadow-[0_8px_30px_rgb(0,0,0,0.04)] hover:border-[#3525cd]/40 hover:bg-slate-50/20 active:scale-[0.98] cursor-pointer transition-all flex flex-col justify-between h-56"
                >
                  <div className="space-y-4">
                    <div className="flex justify-between items-start">
                      <div className="w-12 h-12 bg-indigo-50 text-[#3525cd] rounded-2xl flex items-center justify-center font-black">
                        {stream.name.slice(-1)}
                      </div>

                      <span className="px-3 py-1 bg-indigo-55/10 text-[#3525cd] bg-slate-50 border border-slate-100 rounded-full text-[10px] font-black uppercase font-mono">
                  ...
                  <div className="pt-4 border-t border-slate-100 flex justify-between items-center text-xs">
                    ...
                    <span className="text-[#3525cd] font-bold flex items-center gap-1 group-hover:underline">
            ...
            <form onSubmit={handleAssignTeacher} className="p-6 space-y-4">
              ...
              <div className="pt-4 flex gap-3">
                ...
                <button
                  ...
                  className="flex-1 py-2.5 bg-[#3525cd] hover:bg-[#4f46e5] text-white rounded-xl text-xs font-bold flex items-center justify-center gap-1 cursor-pointer transition-all"
                >
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1.5">
                  ...
                  <select
                    ...
                    className="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-xl focus:border-[#3525cd] outline-none text-xs font-semibold text-slate-800"
                  >
