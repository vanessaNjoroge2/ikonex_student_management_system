...
export class DashboardService {
  ...
  static async getStats(teacherId: string, isAdmin: boolean = false) {
    const studentWhere: any = { isDeleted: false };
    ...
    const studentsPerForm: Record<string, number> = {};
    for (const form of forms) {
      studentsPerForm[form] = await prisma.student.count({
    ...
    const studentsPerStream: Record<string, number> = {};
    for (const str of streamsList) {
      studentsPerStream[str] = await prisma.student.count({
    ...
    for (const status of statuses) {
      statusCounts[status] = await prisma.student.count({
        where: { ...studentWhere, enrollmentStatus: status },
      });
    }

    const scores = await prisma.score.findMany({ where: scoreWhere });
  ...
  static async getActivity(teacherId: string, isAdmin: boolean = false) {
    const studentWhere: any = { isDeleted: false };
    const scoreWhere: any = {};
