...
import { getGradeLetter } from '../../utils/grading';
import { ExamType } from '@prisma/client';

export class ReportsService {
  static async getStudentReport(studentId: string, term?: string, year?: number) {
  ...
  static async getClassSummary(streamId: string, subjectId?: string, term?: string, year?: number) {
    const subjects = await prisma.subject.findMany();
    const students = await prisma.student.findMany({ where: { streamId } });
    ...
    for (const sub of subjects) {
      ...
      if (scores.length > 0) {
        ...
        const stream = await StreamsService.getStream(streamId);
  ...
  static async getStreamComparison(term?: string, year?: number) {
    const subjects = await prisma.subject.findMany();
    const streams = await prisma.stream.findMany();

    const result = [];
    for (const sub of subjects) {
      const getStreamAverage = async (streamId: string) => {
        const scores = await prisma.score.findMany({
          where: {
            subjectId: sub.id,
            student: { streamId },
  ...
  static async getGradeDistribution(subjectId?: string, streamId?: string, term?: string, year?: number) {
    ...
    for (const student of students) {
      const gradesResult = await StudentsService.computeGrades(student.id);
      ...
      filtered.forEach((g) => {
        if (g.letter.startsWith('A')) countA++;
        else if (g.letter.startsWith('B')) countB++;
        else if (g.letter.startsWith('C')) countC++;
        else if (g.letter.startsWith('D')) countD++;
        else countF++;
      });
    }
  ...
  static async getTopPerformers(term?: string, year?: number, limit = 10) {
    const students = await prisma.student.findMany({
  ...
  static async getTermTrend(streamId: string, subjectId?: string) {
    const stream = await StreamsService.getStream(streamId);
